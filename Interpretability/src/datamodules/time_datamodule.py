...
class TimeDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        if not self.train_dataset and not self.val_dataset and not self.test_dataset:
            ...
            train_data = np.concatenate((train_data, test_data[:round(0.63*len(test_data))]))
            train_target = np.concatenate((train_target, test_target[:round(0.63*len(test_target))]))
